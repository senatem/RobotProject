...
namespace RobotProject
{
    ...
    public class OffsetCalculator
    {
        ...
        {
            return new Offsets(0, 0 + ((palletWidth * 10) % 200) / 2, pz * counter + palletHeight, 1, counter);
        }
        ...
        private static Offsets PatternTwo(int px, int pz, int counter, int palletHeight, int palletWidth)
        {
            ...
            {
                0 => new Offsets(-px - 30, 0 + ((palletWidth * 10) % 200) / 2, pz * ((counter + 1) / 2) + palletHeight, 2, (counter + 1) / 2),
                1 => new Offsets(0, 0 + ((palletWidth * 10) % 200) / 2, pz * ((counter + 1) / 2) + palletHeight, 2, (counter + 1) / 2),
                _ => throw new ArgumentOutOfRangeException()
            };
        }
        ...
        private static Offsets PatternFive(int py, int pz, int counter, int palletHeight, int palletWidth)
        {
            return (counter % 2) switch
            {
                0 => new Offsets(0 - 30, py/2 + ((palletWidth * 10) % 200) / 2, pz * ((counter + 1) / 2) + palletHeight, 2, (counter + 1) / 2),
                1 => new Offsets(0, py/2 + ((palletWidth * 10) % 200) / 2, pz * ((counter + 1) / 2) + palletHeight, 2, (counter + 1) / 2),
                _ => throw new ArgumentOutOfRangeException()
            };
        }
    }
